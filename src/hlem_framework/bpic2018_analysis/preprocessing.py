...
def rename_resources(log):
    """
    Renames resources from 'E: org:role' to 'org:resource'.
    
    :param log: The input event log.
    :return: The modified event log with renamed resources.
    """
    logging.info("Renaming resources from 'E: org:role' to 'org:resource'")
    for trace in log:
        for event in trace:
            role = event.get('org:role')
            if role:
                event['org:resource'] = role
                del event['org:role']
...
def partition_outcome(log):
    ...
    A successful case contains the activity 'APayment handled'.
    ...
    for i, trace in enumerate(log):
        # if any((event.get('concept:name') or '').lower() == 'clear invoice' for event in trace):
        if any((event.get('concept:name') or '') == 'finish payment' for event in trace):
...
def partition_on_throughput(log):
    ...
    :param log: The input event log.
    :return: class_under_5, class_5_to_10, class_over_10
    """
    class_under_5 = []
    class_5_to_10 = []
    class_over_10 = []
    ...
    logging.info(f"Total cases under 5: {len(class_under_5)}")
    logging.info(f"Total cases between 5 and 10: {len(class_5_to_10)}")
    logging.info(f"Total cases over 10: {len(class_over_10)}")
    return class_under_5, class_5_to_10, class_over_10

def partition_on_throughput_2cats(log):
    """
    Partitions the event log into three throughput time categoeries:
    - <= 10 days;
    - 10 > t >= 30 days;
    ...
    logging.info(f"Total cases under 250: {len(class_under_250)}")
    logging.info(f"Total cases over 10: {len(class_over_250)}")
...
def filter_incomplete_cases(log):
    ...
    """
    # BPI 2019
    # completion_activities = ['Clear Invoice', 'Record Invoice Receipt', 'Record Goods Receipt', 'Delete Purchase Order Item', 'Remove Payment Block']
    
    # BPI 2018
    completion_activities = ['finish payment']
