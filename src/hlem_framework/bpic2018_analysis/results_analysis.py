...
def throughput_tables(result_df, outcome_throughput):
    ...
    header = ['Length', 'Frequency', 'Path', 'Part&under5', 'Part&over10', 'NonPart&under5',
               'NonPart&over10', 'p']
    rows = []
    for i in range(len(result_df)):
        ...
        if special_throughput:
            ...
            row = [len(path), path_freq, path, part_and_class1, part_and_class2, non_part_and_class1,
                   non_part_and_class2, p_value]
            rows.append(row)
